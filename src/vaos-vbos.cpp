...
void DescrVBOAtribs::crearVBO() 
{
   ...
   glGenBuffers(1, &buffer); //crea un VBO, produce buffer >0

   // 2. fija este buffer como buffer 'activo' actualmente en el 'target' GL_ARRAY_BUFFER
   glBindBuffer(GL_ARRAY_BUFFER, buffer); //activamos el VBO creado vacío
   
   // 3. transferir los datos desde la memoria de la aplicación al VBO en GPU
   glBufferData(GL_ARRAY_BUFFER, tot_size, data, GL_STATIC_DRAW); //reserva memoria para el VBO activo y transfiere un bloque de bytes desde la RAM hacia dicha memoria
   
   // 4. registrar, para este índice de atributo, la localización y el formato de la tabla en el buffer 
   glVertexAttribPointer(index,size,type,GL_FALSE,stride,offset); //en nuestro caso stride y offset es 0
   
   // 5. desactivar el buffer
   glBindBuffer(GL_ARRAY_BUFFER,0);

   // 6. habilitar el uso de esta tabla de atributos en el VAO activo
...
void DescrVAO::crearVAO()
{
   ...
   glGenVertexArrays(1,&array); //produce array>0, crea un nuevo nombre de VAO
   glBindVertexArray(array); //activa el VAO anterior
   ...
   //'dvbo_atributo'=vector de punteros a los descriptores de VBOs de atributos
   //ejemplo: si num_atribs=4: dvbo_atributo[0]=posicion, dvbo_atributo[1]=color, dvbo_atributo[2]=normal, dvbo_atributo[3]=textura...
   ...
   //solo hay un unico VBO de índices (si hay índices) porque simplemente señalan la posición de los vértices en los VBOs de atributos
...
void DescrVAO::draw( const GLenum mode )
{
   ...
   // 
   if (array>0){  //si ya se ha creado
      glBindVertexArray(array);
   }else{   //si no se ha creado array==0
      crearVAO();
   } 
   ...
   //
   if (dvbo_indices==nullptr){   //si no es indexada
